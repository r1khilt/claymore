...
| Messaging | **Telegram** (hackathon + pilot, free, instant) | Twilio SMS **dropped for hackathon** — A2P 10DLC carrier review (10–15 days) exceeds the timeline; revisit post-hackathon. Optional second demo surface: WhatsApp via Twilio sandbox (instant, no carrier approval) or Slack DM (Composio Slack app already connected). iMessage via Sendblue/Loop later. |
...
Messaging decision (2026-07-08): **Twilio A2P 10DLC is dropped for the hackathon** — carrier review (10–15 days) ends after the hackathon does. All messaging runs on **Telegram**; WhatsApp-via-Twilio-sandbox or Slack DM are the optional extra demo surfaces. `messaging/twilio_sms.py` stays a stub behind the `MessagingChannel` port for post-hackathon.
