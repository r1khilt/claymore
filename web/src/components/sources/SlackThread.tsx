  ...
  return (
    <div className="mt-0.5 flex flex-wrap items-center gap-1">
      {items.map((r, i) => (
        <span
          key={i}
          className="flex items-center gap-0.5 rounded-full border border-slack-blue/35 bg-slack-blue/10 px-1.5 py-[1px] text-[10px] font-semibold text-slack-blue"
        >
...
function ThreadReplies({ replies }: { replies: NonNullable<SourceMessage['replies']> }) {
  return (
    <button
      type="button"
      className="mt-1 flex w-full items-center gap-1.5 rounded-lg border border-line/80 bg-white/40 px-2 py-0.5 text-left transition hover:border-line-strong hover:bg-white/70 hover:shadow-sm"
    >
      <span className="flex -space-x-1">
        {replies.by.slice(0, 3).map((p, i) => (
          <Avatar
            ...
            className="rounded-[4px] ring-2 ring-white"
          />
        ))}
      </span>
      <span className="text-[11px] font-bold text-slack-blue">
        {replies.count} {replies.count === 1 ? 'reply' : 'replies'}
      </span>
      <ChevronRight className="ml-auto size-3 text-faint" strokeWidth={2} />
    </button>
  )
}
...
function Body({ m }: { m: SourceMessage }) {
  return (
    <>
      <div className="text-[12.5px] leading-[1.4] text-ink">{renderText(m.text)}</div>
        ...
        return (
          <Fragment key={m.id}>
            ...
            {grouped ? (
              ...
              <div className={cn('flex items-start gap-2 px-1', !newDay && 'mt-2')}>
                <div className="relative mt-0.5 shrink-0">
                  <Avatar name={m.author} accent={m.accent} size={26} photo={photoForAuthor(m.author)} className="rounded-[6px]" />
                  <span className="absolute -bottom-[3px] -right-[3px] size-[9px] rounded-full bg-slack-active ring-2 ring-white" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-baseline gap-1.5">
                    ...
                    <span className="text-[10.5px] text-muted">{clockTime(m.timestamp)}</span>
                  </div>
                  <Body m={m} />
                </div>
              </div>
