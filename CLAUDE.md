...
| Messaging | **WhatsApp** (hackathon surface, decided 2026-07-09) | Replaces Telegram. Twilio SMS/10DLC dropped (carrier review outlasts the hackathon). WhatsApp path TBD at build time: Twilio WhatsApp **sandbox** (instant, no Meta approval — recommended for the demo) vs Composio/Meta WhatsApp Business (needs Meta approval, slower). Implement behind the `MessagingChannel` port so the path is swappable. iMessage via Sendblue/Loop later. |
...
Messaging decision (updated 2026-07-09): the hackathon surface is **WhatsApp** (was Telegram). Twilio A2P 10DLC SMS stays dropped (carrier review outlasts the hackathon). WhatsApp implementation path is chosen at build time — Twilio WhatsApp **sandbox** (instant, no Meta Business approval; recommended) vs a Meta/Composio WhatsApp Business integration (approval-gated, slower). Build it behind the `MessagingChannel` port so the path is swappable; `messaging/twilio_sms.py` remains a post-hackathon stub.
